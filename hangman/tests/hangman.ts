...
describe("hangman", () => {
  ...
  const Keypair_Secretkey = [
    98, 78, 9, 165, 0, 44, 44, 82, 240, 225, 32, 88, 51, 13, 104, 18, 235, 23,
    94, 254, 68, 202, 222, 75, 104, 206, 48, 189, 149, 62, 130, 128, 107, 41,
    42, 111, 130, 192, 83, 78, 75, 36, 153, 84, 242, 169, 58, 148, 1, 20, 41,
    115, 253, 49, 250, 41, 99, 178, 226, 70, 106, 132, 5, 102,
  ];
  const Keypair = anchor.web3.Keypair.fromSecretKey(
    new Uint8Array(Keypair_Secretkey)
  );
  ...
  it("Guess Correct Letters & Get Pay Out", async () => {
    ...
    for (let i = 0; i < letters_to_guess.length; i++) {
      ...
      try {
        await program.methods
          .addCorrectLetter(letters_to_guess[i])
          .accounts({
            gameDataAccount: newGameDataAccount,
            chestVaultAccount: chestVaultAccount,
            player: hot_pubkey,
